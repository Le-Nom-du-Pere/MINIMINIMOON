...
class ErrorClassifier:
    ...
    @staticmethod
    def classify_error(exception: Exception) -> Tuple[ErrorType, Union[TransientErrorType, PermanentErrorType]]:
        """
        ...
        if ("resource temporarily unavailable" in error_message or 
            "device busy" in error_message or
            "try again" in error_message):
            return ErrorType.TRANSIENT, TransientErrorType.TEMPORARY_RESOURCE_UNAVAILABLE
        
        # I/O errors that might be transient
        if isinstance(exception, (OSError, IOError)) and exception.errno in [11, 35, 115]:  # EAGAIN, EWOULDBLOCK, EINPROGRESS
        ...
        if ("pdf" in error_message and ("corrupt" in error_message or 
                                       "malformed" in error_message or
        ...
        if ("format" in error_message and "invalid" in error_message) or "parse error" in error_message:
        ...
        if isinstance(exception, ValueError) and ("format" in error_message or "invalid" in error_message):
            return ErrorType.PERMANENT, PermanentErrorType.INVALID_FORMAT
    ...
    def log_error(self, error: PlanProcessingError) -> str:
        ...
        try:
            with open(log_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Error Type: {error.error_type.value} ({error.specific_error.value})\n")
...
class PlanProcessor(ABC):
    ...
    def __init__(self, retry_config: Optional[RetryConfig] = None, log_directory: str = "error_logs"):
    ...
    def process_plan(self, plan_data: Dict[str, Any], plan_id: Optional[str] = None) -> ProcessingResult:
        """
        ...
        if plan_id is None:
            ...
            plan_id = hashlib.md5(json.dumps(plan_data, sort_keys=True).encode()).hexdigest()[:8]
    ...
    def batch_process_plans(self, plans: List[Tuple[Dict[str, Any], Optional[str]]]) -> List[ProcessingResult]:
        """
        ...
        self.error_logger.logger.info(f"Starting batch processing of {len(plans)} plans")
...
class FeasibilityPlanProcessor(PlanProcessor):
    ...
    def __init__(self, retry_config: Optional[RetryConfig] = None, log_directory: str = "error_logs"):
        ...
        try:
            ...
            self.scorer = FeasibilityScorer(enable_parallel=False)  # Disable parallel in processor
        except ImportError:
            self.scorer = None
            self.error_logger.logger.warning("FeasibilityScorer not available, using mock implementation")
    ...
    def _process_plan_implementation(self, plan_data: Dict[str, Any], plan_id: str) -> Dict[str, Any]:
        ...
        if not indicators:
            raise FileNotFoundError("No indicators found in plan data")  # Use permanent error type
            ...
            for indicator in indicators:
                if isinstance(indicator, str):
                    ...
                    results.append({
                        ...
                        "components_detected": [c.value for c in score_result.components_detected],
                    ...
                    raise FileNotFoundError(f"Invalid indicator format: {type(indicator)}")  # Use permanent error
        else:
            # Mock implementation
            results = []
            for indicator in indicators:
        ...
        return {
            ...
            "average_score": sum(r["feasibility_score"] for r in results) / len(results),
