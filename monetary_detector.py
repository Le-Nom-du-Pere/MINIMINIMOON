...
class MonetaryDetector:
    ...
    def _setup_patterns(self):
        ...
        currency_symbols = (
            r"[\$€£¥]|USD|COP|EUR|GBP|JPY|CHF|CAD|AUD|MXN|PEN|CLP|ARS|BRL"
        )
    ...
    def _extract_currency(
        self, pre_currency: Optional[str], post_currency: Optional[str]
    ) -> Optional[str]:
    ...
    def _apply_scale(self, value: float, scale_str: Optional[str]) -> float:
        ...
        scale_key = (
            scale_str if scale_str in self.scale_multipliers else scale_str.lower()
        )
        ...
        for match in self.monetary_pattern.finditer(normalized_text):
            ...
            try:
                ...
                currency = self._extract_currency(
                    match.group("pre_currency"), match.group("post_currency")
                )
