...
class TeamCitySetupTester:
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(self.venv_path):
                ...
                if os.path.exists(python_exec):
                    self.log_test("Virtual Environment Creation", True, f"Created at {self.venv_path}")
                    return True
                else:
                    self.log_test("Virtual Environment Creation", False, "Python executable not found")
                    return False
            else:
    ...
    def test_environment_isolation(self):
        ...
        try:
            ...
            if not os.path.exists(python_exec):
                self.log_test("Environment Isolation", False, "Virtual environment not found")
                return False
            ...
            if result.returncode == 0:
                ...
                if "PYTHONNOUSERSITE=1" in output and "UserSite: True" in output:
                    self.log_test("Environment Isolation", True, "PYTHONNOUSERSITE working")
                    return True
                else:
                    self.log_test("Environment Isolation", False, f"Unexpected output: {output}")
                    return False
            else:
    ...
    def test_pip_caching(self):
        ...
        try:
            ...
            cmd = [pip_exec, "install", "--cache-dir", cache_dir, "--dry-run", "setuptools"]
            result = subprocess.run(cmd, capture_output=True, text=True, 
                                  env=env, timeout=30)
    ...
    def test_project_dependencies(self):
        ...
        try:
            ...
            if not os.path.exists(requirements_file):
                self.log_test("Project Dependencies", False, "requirements.txt not found")
                return False
            ...
            cmd = [pip_exec, "install", "--cache-dir", cache_dir, "-r", test_requirements]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            
            if result.returncode == 0:
                self.log_test("Project Dependencies", True, "Dependencies installed successfully")
                return True
            else:
    ...
    def test_module_import(self):
        ...
        try:
            ...
            cmd = [python_exec, "-c", "import feasibility_scorer; print('Import successful')"]
            result = subprocess.run(cmd, capture_output=True, text=True, 
                                  env=env, timeout=10, cwd=self.temp_dir)
            
            if result.returncode == 0 and "Import successful" in result.stdout:
                self.log_test("Module Import", True, "feasibility_scorer imported successfully")
                return True
            else:
...
"""]
            ...
            if result.returncode == 0 and "Build validation passed" in result.stdout:
                ...
                self.log_test("Build Validation", False, f"Validation failed: {result.stderr}")
                return False
        except Exception as e:
