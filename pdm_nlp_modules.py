...
class ContradictionMatch(BaseModel):
    ...
    hypothesis_location: Dict[str, int] = Field(description="Start/end positions")
...
class CompetenceValidation(BaseModel):
    ...
    level: str = Field(description="Government level (municipal/departmental/national)")
    ...
    # Evidence
    legal_basis: List[str] = Field(default_factory=list, description="Legal references")
    ...
    def find_adversatives(self, text: str, context_window: int = 200) -> List[Dict]:
        ...
        for pattern in self.adversatives:
            for match in pattern.finditer(text):
                ...
                quants_found = self._find_in_context(context, self.quantitative)
                modals_found = self._find_in_context(context, self.modals)
                negations_found = self._find_in_context(context, self.negations)
                
                # Calculate complexity score
                complexity = len(goals_found) + len(verbs_found) + len(quants_found)
