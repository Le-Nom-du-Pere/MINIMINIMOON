...
def extract_numerical_data(report_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'resumen_ejecutivo' in report_data:
        ...
        numerical_data['dimensiones_evaluadas'] = resumen.get('dimensiones_evaluadas', 0)
    ...
    if 'dimensiones' in report_data:
        ...
        for dim in report_data['dimensiones']:
            dim_numerical = {
                ...
                'consistencia_logica': dim.get('evaluacion_causal', {}).get('consistencia_logica', 0),
                'identificabilidad_causal': dim.get('evaluacion_causal', {}).get('identificabilidad_causal', 0),
                'factibilidad_operativa': dim.get('evaluacion_causal', {}).get('factibilidad_operativa', 0),
                'certeza_probabilistica': dim.get('evaluacion_causal', {}).get('certeza_probabilistica', 0),
                'robustez_causal': dim.get('evaluacion_causal', {}).get('robustez_causal', 0),
                'evidencia_soporte': dim.get('evaluacion_causal', {}).get('evidencia_soporte', 0),
            }
            ...
            if 'evidencia' in dim:
                for tipo_evidencia, evidencias in dim['evidencia'].items():
                    if isinstance(evidencias, list):
                        scores = [ev.get('score_final', 0) for ev in evidencias if isinstance(ev, dict)]
                        if scores:
...
def compare_numerical_data(data1: Dict[str, Any], data2: Dict[str, Any], tolerance: float = 1e-10) -> Dict[str, Any]:
    """
    ...
    for key in ['puntaje_global', 'dimensiones_evaluadas']:
        if key in data1 and key in data2:
            ...
            if abs(val1 - val2) > tolerance:
                differences.append(f"{key}: {val1} vs {val2} (diff: {abs(val1 - val2)})")
                ...
                for key in dim1.keys():
                    if key in dim2 and key.endswith('_scores'):
                        ...
                        if dim1[key] != dim2[key]:
                            differences.append(f"Dimensión {i}, {key}: {dim1[key]} vs {dim2[key]}")
                    elif key in dim2 and isinstance(dim1[key], (int, float)):
                        # Comparar valores numéricos
                        if abs(dim1[key] - dim2[key]) > tolerance:
                            differences.append(f"Dimensión {i}, {key}: {dim1[key]} vs {dim2[key]} (diff: {abs(dim1[key] - dim2[key])})")
    
    return {
        'identical': len(differences) == 0,
        'differences_count': len(differences),
        'differences': differences[:10]  # Limitar a primeras 10 diferencias para legibilidad
    }
...
def create_test_pdf() -> Path:
    ...
    try:
        ...
        c.drawString(100, 720, "1. Prevención de la violencia y protección frente al conflicto armado")
        c.drawString(100, 690, "Se implementarán estrategias integrales para reducir los índices de violencia")
        c.drawString(100, 660, "mediante fortalecimiento institucional y participación ciudadana.")
        c.drawString(100, 630, "2. Equidad de género en acceso a servicios y oportunidades")
        c.drawString(100, 600, "Promoción de políticas públicas con enfoque diferencial de género")
        c.drawString(100, 570, "para garantizar igualdad de oportunidades.")
        c.drawString(100, 540, "3. Desarrollo económico sostenible")
        c.drawString(100, 510, "Fomento del emprendimiento y fortalecimiento del tejido empresarial local.")
        c.save()
...
def run_evaluation_with_seed(pdf_path: Path, seed: int) -> Dict[str, Any]:
    ...
    # Ejecutar evaluación
    cmd = [sys.executable, "Decatalogo_principal.py", "--seed", str(seed), str(pdf_path)]
