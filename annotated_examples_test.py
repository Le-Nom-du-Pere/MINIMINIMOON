...
class TestAnnotatedExamples(unittest.TestCase):
        ...
        self.annotated_examples = {
            ...
            "multiple_patterns_mixed": {
                ...
                "expected_causal": [
                    ...
                    # Instrumental: monitoring tool usage
                    ("mediante", False, 0.25),
                    (
                        "tendencia a",
                        False,
    ...
    def test_precision_vs_recall_balance(self):
        ...
        for example in all_examples:
            ...
            for expected_connector, is_causal, _ in example["expected_causal"]:
                ...
                # Check if detected
                detected = any(
                    m.connector == expected_connector for m in matches)
                if detected:
                    if is_causal:
    ...
    def test_confidence_score_correlation(self):
        ...
        for example in self.annotated_examples.values():
            ...
            for match in matches:
                ...
                if expected:
                    all_matches.append(
                        (match.confidence, expected[1], expected[2]))
        ...
        if true_causal_confidences and false_causal_confidences:
            avg_true = sum(true_causal_confidences) / \
                len(true_causal_confidences)
            avg_false = sum(false_causal_confidences) / \
                len(false_causal_confidences)
    ...
    def test_new_patterns_weighted_correctly(self):
        ...
        self.assertGreater(
            strong_matches[0].confidence, weak_matches[0].confidence)
