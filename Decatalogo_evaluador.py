...
assert sys.version_info >= (3, 11), "Python 3.11+ required for production deployment"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('decatalogo_evaluator.log'),
...
class AnalisisEvidenciaDecalogo:
    ...
    def max_score(self) -> float:
        ...
        return max(
            (score.feasibility_score for score in self.indicador_scores),
            default=0.0
        )
...
class IndustrialDecatalogoEvaluatorFull(_EvaluadorBase):
    ...
    def _extraer_texto_entry(self, entry: Any) -> str:
        ...
        if isinstance(entry, dict):
            ...
            for key in ["texto", "texto_evidencia", "segmento", "contenido", "descripcion", "value"]:
                valor = entry.get(key)
                if valor:
                    return str(valor)
    ...
    def _analizar_evidencia(
        ...
        textos_indicadores = self._extraer_textos(
            evidencia,
            "indicadores", "metas", "productos", "resultados", "impactos"
        )

        if textos_indicadores:
            # Usar batch_score para eficiencia
            batch_result = self.scorer.batch_score(textos_indicadores, use_parallel=True)
        ...
        textos_responsables = self._extraer_textos(
            evidencia,
            "responsables", "actores", "entidades", "programas", "proyectos"
        )
        ...
        textos_analisis = self._extraer_textos(
            evidencia,
            "diagnostico", "objetivos", "estrategias", "justificacion"
        )
        ...
        for texto in textos_analisis[:10]:  # Limitar para performance
            analisis_contradiccion = self.contradiction_detector.detect_contradictions(texto)
            if analisis_contradiccion.total_contradictions > 0:
                todas_contradicciones.extend(analisis_contradiccion.contradictions)
                # Mantener el análisis más severo
                if not analisis_contradicciones_global or analisis_contradiccion.risk_score > analisis_contradicciones_global.risk_score:
        ...
        if self.monetary_detector:
            textos_presupuesto = self._extraer_textos(
                evidencia,
                "presupuesto", "recursos", "financiacion", "inversion"
            )
            for texto in textos_presupuesto:
                matches = self.monetary_detector.detect_monetary_expressions(texto)
                valores_monetarios.extend(matches)

        # 6. CALCULAR MÉTRICAS AGREGADAS
        recursos = len(valores_monetarios) + len(evidencia.get("presupuesto", []))
        ...
        confidence_global = np.mean(confidence_scores) if confidence_scores else 0.0

        # 8. METADATA DEL ANÁLISIS
        metadata_analisis = {
            "total_textos_analizados": len(textos_indicadores) + len(textos_responsables) + len(textos_analisis),
            "indicadores_de_calidad_alta": sum(1 for score in indicador_scores if score.feasibility_score >= 0.8),
            "responsables_identificados": len(set(r.text for r in responsabilidades)),
            "contradicciones_criticas": sum(
                1 for c in todas_contradicciones if c.risk_level.value in ["high", "medium-high"]),
    ...
    def evaluar_dimension_de1(
        ...
        evaluaciones.append(
            self._crear_evaluacion(
                "Q1", "DE-1", punto_id, valor_q1,
                self._formatear_deteccion(self._seleccionar_mejor_deteccion(productos_detections)),
        ...
        if analisis.responsabilidades:
            ...
            institucionales = [r for r in analisis.responsabilidades if
                               r.entity_type in [EntityType.ORGANIZATION, EntityType.POSITION]]
            valor_q2 = min(1.0, len(institucionales) * 0.25) if institucionales else 0.1

        evaluaciones.append(
            self._crear_evaluacion(
                "Q2", "DE-1", punto_id, valor_q2,
                self._formatear_responsable(self._seleccionar_mejor_responsable(analisis.responsabilidades)),
        ...
        # Criterios causales
        coherencia = np.mean([puntajes[0], puntajes[1]]) if puntajes[:2] else 0.0
        articulacion = np.mean([puntajes[2], puntajes[3]]) if len(puntajes) > 3 else 0.0
        consistencia = np.mean([puntajes[4], puntajes[5]]) if len(puntajes) > 5 else 0.0
        suficiencia = analisis.confidence_global

        # Ajuste por contradicciones
        if analisis.analisis_contradicciones:
            factor_contradiccion = 1 - (analisis.analisis_contradicciones.risk_score * 0.3)
            coherencia *= factor_contradiccion
            articulacion *= factor_contradiccion

        puntaje_causalidad = (coherencia + articulacion + consistencia + suficiencia) / 4
        ...
        puntaje_dimension = (
                puntaje_bienestar * factor_causal +
                puntaje_resultado +
                puntaje_gestion
        )
    ...
    def evaluar_dimension_de2(
        ...
        valor_d2 = min(1.0, dates_found / 5) if dates_found >= 5 else dates_found / 10
        valores_diagnostico.append(valor_d2)

        # D3-D6: Análisis de causas y brechas
        for key in ["causas_directas", "causas_estructurales", "brechas_territoriales", "grupos_poblacionales"]:
            elementos = evidencia.get(key, [])
        ...
        claves_alineacion = [
            "objetivos_pnd", "indicadores_transformadores", "problemas_estructurales",
            "ods", "vision_largo_plazo", "determinantes_ambientales"
        ]
        ...
        for clave in claves_alineacion:
            ...
            if clave == "ods":
                ...
                valor = min(1.0, len(elementos) / 3) if len(elementos) >= 3 else len(elementos) / 6
            else:
        ...
        for key in ["codigo_bpin", "presupuesto_plurianual", "fuentes_financiacion",
                    "localizacion", "articulacion_proyectos", "participacion_formulacion"]:
            elementos = evidencia.get(key, [])
        ...
        todos_valores = valores_diagnostico + valores_alineacion + valores_territorializacion

        for idx, (pregunta_id, pregunta_texto) in enumerate(preguntas[:len(todos_valores)]):
            valor = todos_valores[idx] if idx < len(todos_valores) else 0.0
            evaluaciones.append(
                self._crear_evaluacion(
                    pregunta_id, "DE-2", punto_id, valor,
        ...
        puntaje_diagnostico = np.mean(valores_diagnostico) * 25 if valores_diagnostico else 0
        puntaje_alineacion = np.mean(valores_alineacion) * 30 if valores_alineacion else 0
        puntaje_territorializacion = np.mean(valores_territorializacion) * 25 if valores_territorializacion else 0
        ...
        return EvaluacionDimensionPunto(
            ...
            matriz_causal={
                ...
                "completitud_tematica": round(sum(todos_valores) / len(todos_valores) if todos_valores else 0, 3),
            },
    ...
    def evaluar_dimension_de3(
        ...
        for idx, (pregunta_texto, (clave_evidencia, peso_base)) in enumerate(zip(self.preguntas_de3, mapeo_evidencia)):
            ...
            if elementos:
                ...
                if any("acta" in str(e).lower() or "informe" in str(e).lower() for e in elementos):
                    valor = min(1.0, valor + 0.1)
            else:
                valor = 0.0
        ...
        gobernanza_score = sum(v > 0.5 for v in valores_participacion) / len(valores_participacion)
        mecanismos_formales = sum(1 for v in valores_participacion[:3] if v > 0)
        mecanismos_digitales = sum(1 for v in valores_participacion[3:] if v > 0)
        ...
        return EvaluacionDimensionPunto(
            ...
            matriz_causal={
                ...
                "actores_identificados": len(set(r.text for r in analisis.responsabilidades)),
                "calidad_documental": round(
                    np.mean([v for v in valores_participacion if v > 0]) if any(valores_participacion) else 0, 3),
            },
    ...
    def evaluar_dimension_de4(
        ...
        coverage_productos = min(1.0, len([s for s in analisis.indicador_scores if s.has_quantitative_target]) * 0.1)
        coverage_resultados = analisis.max_score
        coverage_impactos = coverage_resultados * 0.8 if coverage_resultados > 0.5 else 0

        # Factor de riesgo basado en contradicciones
        if analisis.analisis_contradicciones:
            riesgo_factor = 1 - (analisis.analisis_contradicciones.risk_score * 0.4)
        else:
        ...
        valor_r2 = coverage_resultados if analisis.tiene_linea_base else coverage_resultados * 0.5
        ...
        for idx, (pregunta_texto, valor_base) in enumerate(zip(self.preguntas_de4, valores_preguntas)):
            valor_ajustado = valor_base * riesgo_factor

            evaluaciones.append(
                self._crear_evaluacion(
                    f"R{idx + 1}", "DE-4", punto_id,
                    valor_ajustado,
        ...
        return EvaluacionDimensionPunto(
            ...
            matriz_causal={
                ...
                "indicadores_cuantitativos": sum(1 for s in analisis.indicador_scores if s.has_quantitative_target),
    ...
    def evaluar_punto_completo(
        ...
        for dim in evaluaciones_dimensiones:
            nivel = "alto" if dim.puntaje_dimension >= 70 else "medio" if dim.puntaje_dimension >= 40 else "bajo"
            explicacion_partes.append(
        ...
        # Análisis de riesgos
        if analisis.riesgos > 0:
            nivel_riesgo = "crítico" if analisis.riesgos > 5 else "moderado" if analisis.riesgos > 2 else "bajo"
            explicacion_partes.append(
        ...
        for dim in evaluaciones_dimensiones:
            ...
            for evaluacion in dim.evaluaciones_preguntas:
                if evaluacion.puntaje >= 0.8:
                    fortalezas.append(f"{evaluacion.pregunta_id}: {evaluacion.evidencia_textual[:50]}...")
        ...
        if not analisis.tiene_linea_base:
            riesgos_detectados.append("Ausencia de línea base dificulta medición de avances")

        if not analisis.responsabilidades:
            riesgos_detectados.append("Sin responsables institucionales claros para implementación")

        # CONSTRUIR RESULTADO INDUSTRIAL
        nombre_punto = self.contexto.dimension_por_id.get(
            punto_id,
            DimensionDecalogo(punto_id, f"Punto {punto_id} del Decálogo", "")
        ).nombre
        ...
        resultado_industrial = ResultadoDimensionIndustrial(
            ...
            componentes_evaluados={
                dim.dimension: dim.puntaje_dimension
                for dim in evaluaciones_dimensiones
            },
            ...
            metadata={
                ...
                "contradicciones_detectadas": analisis.riesgos,
                "responsables_identificados": len(set(r.text for r in analisis.responsabilidades)),
    ...
    def evaluar_cluster(
        ...
        for punto_id in cluster.puntos:
            if punto_id not in evidencias_por_punto:
                LOGGER.warning(f"⚠️ Sin evidencia para punto {punto_id} en cluster {cluster.titulo}")
                continue

            evaluacion_punto, analisis, resultado_industrial = self.evaluar_punto_completo(
        ...
        coherencia_cluster = 1 - (varianza_interna / 100) if varianza_interna < 100 else 0
        ...
        analisis_meso = {
            ...
            "puntos_fuertes": [
                p.nombre_punto for p in evaluaciones_puntos
        ...
        for dimension in ["DE-1", "DE-2", "DE-3", "DE-4"]:
            ...
            if puntajes_dimension:
                analisis_meso["promedio_por_dimension"][dimension] = round(np.mean(puntajes_dimension), 2)

        # Distribución de clasificaciones
        for punto in evaluaciones_puntos:
            clasif = punto.clasificacion_cualitativa
            analisis_meso["distribucion_clasificaciones"][clasif] = (
                    analisis_meso["distribucion_clasificaciones"].get(clasif, 0) + 1
            )
            ...
            for j, punto_j in enumerate(cluster.puntos):
                if i < j:
                    ...
                    if len(puntajes_puntos) > max(i, j):
                        correlacion = 1 - abs(puntajes_puntos[i] - puntajes_puntos[j]) / 100
                        interdependencias[f"{punto_i}-{punto_j}"] = round(correlacion, 3)
    ...
    def _crear_cluster_vacio(self, cluster: ClusterMetadata) -> EvaluacionClusterCompleto:
    ...
    def generar_reporte_final(
        ...
        for cluster in self.contexto.clusters:
            ...
            for eval_punto in eval_cluster.evaluaciones_puntos:
                ...
                todas_recomendaciones.extend(eval_punto.recomendaciones_especificas)

                for eval_dim in eval_punto.evaluaciones_dimensiones:
                    evaluaciones_preguntas.extend(eval_dim.evaluaciones_preguntas)

        # CALCULAR PUNTAJE GLOBAL
        if evaluaciones_clusters:
            puntaje_global = np.mean([c.puntaje_agregado_cluster for c in evaluaciones_clusters])
        else:
            puntaje_global = 0.0
        ...
        resumen_ejecutivo = {
            ...
            "recomendacion_estrategica_global": (
                ...
                else "⚠️ PLAN REQUIERE AJUSTES - Brechas moderadas en alineación con DDHH"
                if puntaje_global >= 50
                else "❌ PLAN REQUIERE REFORMULACIÓN - Brechas críticas en DDHH"
            ),
            "nivel_confianza_evaluacion": round(
                np.mean(
                    [p.confidence for p in evaluaciones_preguntas if hasattr(p, 'confidence') and p.confidence > 0]),
                3
            ) if evaluaciones_preguntas else 0.0,
        ...
        estadisticas_globales = {
            "distribucion_puntajes": {
                "media": round(np.mean([p.puntaje_agregado_punto for p in evaluaciones_puntos]),
                               2) if evaluaciones_puntos else 0,
                "mediana": round(np.median([p.puntaje_agregado_punto for p in evaluaciones_puntos]),
                                 2) if evaluaciones_puntos else 0,
                "desviacion_estandar": round(np.std([p.puntaje_agregado_punto for p in evaluaciones_puntos]),
                                             2) if evaluaciones_puntos else 0,
                "minimo": round(min([p.puntaje_agregado_punto for p in evaluaciones_puntos]),
                                2) if evaluaciones_puntos else 0,
                "maximo": round(max([p.puntaje_agregado_punto for p in evaluaciones_puntos]),
                                2) if evaluaciones_puntos else 0,
            },
            "distribucion_respuestas": {
                "si": sum(1 for q in evaluaciones_preguntas if q.respuesta == "Sí"),
                "parcial": sum(1 for q in evaluaciones_preguntas if q.respuesta == "Parcial"),
            ...
            "indicadores_clave": {
                "puntos_optimos": sum(
                    1 for p in evaluaciones_puntos if p.puntaje_agregado_punto >= self.umbrales["optimo"]),
                "puntos_satisfactorios": sum(1 for p in evaluaciones_puntos if
                                             self.umbrales["satisfactorio"] <= p.puntaje_agregado_punto < self.umbrales[
                                                 "optimo"]),
                "puntos_basicos": sum(1 for p in evaluaciones_puntos if
                                      self.umbrales["basico"] <= p.puntaje_agregado_punto < self.umbrales[
                                          "satisfactorio"]),
                "puntos_insuficientes": sum(
                    1 for p in evaluaciones_puntos if p.puntaje_agregado_punto < self.umbrales["basico"]),
            }
        }
            ...
            if puntajes_dim:
                estadisticas_globales["analisis_dimensional"][dim] = {
                    ...
                    "coeficiente_variacion": round(
                        np.std(puntajes_dim) / np.mean(puntajes_dim) if np.mean(puntajes_dim) > 0 else 0, 3),
                }
        ...
        recomendaciones_unicas = list(dict.fromkeys(todas_recomendaciones))[:20]
        ...
        reporte_macro = {
            ...
            "distribucion_calidad": {
                "optimo": estadisticas_globales["indicadores_clave"]["puntos_optimos"] / len(
                    evaluaciones_puntos) * 100 if evaluaciones_puntos else 0,
                "satisfactorio": estadisticas_globales["indicadores_clave"]["puntos_satisfactorios"] / len(
                    evaluaciones_puntos) * 100 if evaluaciones_puntos else 0,
                "basico": estadisticas_globales["indicadores_clave"]["puntos_basicos"] / len(
                    evaluaciones_puntos) * 100 if evaluaciones_puntos else 0,
                "insuficiente": estadisticas_globales["indicadores_clave"]["puntos_insuficientes"] / len(
                    evaluaciones_puntos) * 100 if evaluaciones_puntos else 0,
            }
        }
            ...
            try:
                ...
                for pregunta in evaluaciones_preguntas:
                    data_matriz.append({
                        ...
                        "Confianza": round(pregunta.confidence * 100, 1) if hasattr(pregunta, 'confidence') else 0.0,
                        "Evidencia": pregunta.evidencia_textual[:150] if pregunta.evidencia_textual else ""
                    })
                ...
                matriz_trazabilidad.to_csv(archivo_csv, index=False, encoding='utf-8-sig')
                LOGGER.info(f"  ✅ Matriz de trazabilidad guardada: {archivo_csv}")
            ...
            "resultados_industriales": [
                {
                    ...
                    "brechas_principales": r.brechas_identificadas[:3] if hasattr(r, 'brechas_identificadas') else [],
                }
                for r in resultados_dimension
            ],
            "estadisticas_evaluacion": {
                "total_evidencias_procesadas": sum(
                    len(evidencias_por_punto.get(i, {}))
                    for i in range(1, 11)
                ),
            ...
            "resumen_clusters": [
                {
                    ...
                    "coherencia": round(c.coherencia_cluster, 3) if hasattr(c, 'coherencia_cluster') else 0,
                    "varianza": round(c.varianza_interna, 2) if hasattr(c, 'varianza_interna') else 0,
                }
        ...
        LOGGER.info(f"✅ Reporte final generado exitosamente para {nombre_plan}")
        LOGGER.info(f"   - Puntaje Global: {puntaje_global:.1f}/100")
        LOGGER.info(f"   - Clasificación: {alineacion}")
        LOGGER.info(f"   - Confianza: {resumen_ejecutivo['nivel_confianza_evaluacion'] * 100:.1f}%")
    ...
    def _generar_analisis_dimensional(self, analisis_dimensional: Dict) -> str:
        ...
        for dim, stats in analisis_dimensional.items():
            interpretacion = "alto" if stats["promedio"] >= 70 else "medio" if stats["promedio"] >= 40 else "bajo"
            consistencia = "alta" if stats["coeficiente_variacion"] < 0.2 else "media" if stats[
                                                                                              "coeficiente_variacion"] < 0.4 else "baja"
            partes.append(
...
def integrar_evaluador_decatalogo(
        sistema: SistemaEvaluacionIndustrial,
    ...
    try:
        ...
        if not resultado_industrial:
            LOGGER.error(f"❌ No se pudo generar resultado para dimensión {dimension.id}")
            return None

        # Enriquecer con matriz de trazabilidad si está disponible
        if hasattr(sistema.extractor, 'generar_matriz_trazabilidad'):
            try:
                matriz = sistema.extractor.generar_matriz_trazabilidad(dimension)
                if matriz is not None:
                    resultado_industrial.matriz_trazabilidad = matriz
            except Exception as e:
                LOGGER.warning(f"⚠️ No se pudo generar matriz trazabilidad: {e}")
        ...
        if cluster_metadata:
            ...
            if hasattr(resultado_industrial, 'metadata'):
                ...
                resultado_industrial.metadata['cluster_id'] = cluster_metadata.cluster_id
        ...
        LOGGER.error(
            f"❌ Error en evaluación de dimensión {dimension.id}: {exc}",
            exc_info=True
        )
        return None
...
def main():
    ...
    for dim in evaluacion.evaluaciones_dimensiones:
        print(f"   {dim.dimension}: {dim.puntaje_dimension:.1f}/100 - "
        ...
        print(f"   - Responsables identificados: {len(set(r.text for r in analisis.responsabilidades))}")
        print(f"   - Contradicciones detectadas: {analisis.riesgos}")
        print(f"   - Línea base presente: {'✅' if analisis.tiene_linea_base else '❌'}")
        print(f"   - Metas identificadas: {'✅' if analisis.tiene_metas else '❌'}")
        ...
        if reporte_final.estadisticas_globales:
            dist = reporte_final.estadisticas_globales.get("distribucion_respuestas", {})
        ...
        for cluster in reporte_final.reporte_meso_por_cluster:
            ...
            print(f"      - Puntaje: {cluster.puntaje_agregado_cluster:.1f}/100")
            print(f"      - Clasificación: {cluster.clasificacion_cualitativa}")
            print(f"      - Coherencia: {cluster.coherencia_cluster:.2%}")

        print(f"\n📋 RECOMENDACIONES GLOBALES TOP 5:")
        print("-" * 50)
        for i, rec in enumerate(reporte_final.reporte_macro["recomendaciones_globales"][:5], 1):
            print(f"   {i}. {rec}")
        ...
        if HAS_PANDAS and reporte_final.matriz_trazabilidad_global is not None:
            ...
            print(f"   ✅ {len(reporte_final.reporte_por_pregunta)} preguntas evaluadas")
            print(f"   ✅ {len(reporte_final.reporte_por_punto)} puntos analizados")
    ...
    def exportar_resultados_json(
            reporte: ReporteFinalDecatalogo,
            archivo: str = "reporte_decatalogo.json"
    ) -> bool:
        """
    ...
    def generar_reporte_markdown(
            reporte: ReporteFinalDecatalogo,
            archivo: str = "reporte_decatalogo.md"
    ) -> bool:
        """
        ...
        try:
            ...
            # Encabezado
            lineas.append(f"# Reporte de Evaluación - {reporte.metadata['nombre_plan']}")
            lineas.append(f"\n**Fecha:** {reporte.metadata['fecha_evaluacion']}")
            lineas.append(f"**Versión:** {reporte.metadata['version_evaluador']}")
            lineas.append("")

            # Resumen Ejecutivo
            lineas.append("## Resumen Ejecutivo")
            lineas.append("")
            lineas.append(f"- **Puntaje Global:** {reporte.resumen_ejecutivo['puntaje_global']}/100")
            lineas.append(f"- **Nivel de Alineación:** {reporte.resumen_ejecutivo['nivel_alineacion']}")
            lineas.append(f"- **Mejor Cluster:** {reporte.resumen_ejecutivo['cluster_mejor_desempeno']}")
            lineas.append(f"- **Cluster Crítico:** {reporte.resumen_ejecutivo['cluster_peor_desempeno']}")
            lineas.append(f"- **Recomendación:** {reporte.resumen_ejecutivo['recomendacion_estrategica_global']}")
            lineas.append("")
            ...
            for cluster in reporte.reporte_meso_por_cluster:
                ...
                lineas.append(f"- Puntaje: {cluster.puntaje_agregado_cluster:.1f}/100")
                lineas.append(f"- Clasificación: {cluster.clasificacion_cualitativa}")
                lineas.append(f"- Coherencia Interna: {cluster.coherencia_cluster:.2%}")

                if cluster.analisis_meso_completo.get("puntos_fuertes"):
                    lineas.append(f"- Puntos Fuertes: {', '.join(cluster.analisis_meso_completo['puntos_fuertes'])}")
                if cluster.analisis_meso_completo.get("puntos_criticos"):
                    lineas.append(f"- Puntos Críticos: {', '.join(cluster.analisis_meso_completo['puntos_criticos'])}")
                lineas.append("")

            # Top Recomendaciones
            lineas.append("## Recomendaciones Prioritarias")
            lineas.append("")
            for i, rec in enumerate(reporte.reporte_macro["recomendaciones_globales"][:10], 1):
            ...
            lineas.append("")
            for i, brecha in enumerate(reporte.reporte_macro["brechas_globales"][:10], 1):
            ...
            if reporte.estadisticas_globales:
                ...
                lineas.append("")
                dist = reporte.estadisticas_globales.get("distribucion_respuestas", {})
                total = sum(dist.values())
                if total > 0:
                    lineas.append("| Tipo Respuesta | Cantidad | Porcentaje |")
                    lineas.append("|----------------|----------|------------|")
                    lineas.append(f"| Sí | {dist.get('si', 0)} | {dist.get('si', 0) / total * 100:.1f}% |")
                    lineas.append(
                        f"| Parcial | {dist.get('parcial', 0)} | {dist.get('parcial', 0) / total * 100:.1f}% |")
                    lineas.append(f"| No | {dist.get('no', 0)} | {dist.get('no', 0) / total * 100:.1f}% |")
                    lineas.append(f"| No Identificado | {dist.get('ni', 0)} | {dist.get('ni', 0) / total * 100:.1f}% |")
                    lineas.append("")

            # Footer
            lineas.append("---")
            lineas.append(f"*Reporte generado automáticamente por el Sistema de Evaluación del Decálogo v3.0*")
        ...
        try:
            ...
            for reporte in reportes:
                fila = {
                    ...
                    "Confianza": reporte.resumen_ejecutivo.get("nivel_confianza_evaluacion", 0),
                }

                # Agregar puntajes por dimensión
                if reporte.estadisticas_globales and "analisis_dimensional" in reporte.estadisticas_globales:
                    for dim, stats in reporte.estadisticas_globales["analisis_dimensional"].items():
                        fila[f"Puntaje_{dim}"] = stats["promedio"]
                        fila[f"Desviacion_{dim}"] = stats["desviacion"]

                # Agregar puntajes por cluster
                for cluster in reporte.reporte_meso_por_cluster:
                    fila[f"Cluster_{cluster.cluster_id}"] = cluster.puntaje_agregado_cluster
            ...
            df["Z_Score"] = (df["Puntaje_Global"] - df["Puntaje_Global"].mean()) / df["Puntaje_Global"].std()
            df["Percentil"] = df["Puntaje_Global"].rank(pct=True) * 100

            # Guardar CSV
            df.to_csv(archivo_salida, index=False, encoding='utf-8-sig')

            LOGGER.info(f"✅ Comparación de {len(reportes)} municipios guardada: {archivo_salida}")
            ...
            for idx, row in df.head(5).iterrows():
                print(f"   {row['Ranking']}. {row['Municipio']}: {row['Puntaje_Global']:.1f}")
    ...
    def validar_integridad_evaluador() -> bool:
        ...
        try:
            ...
            validaciones.append(("Contexto decálogo", tiene_clusters and tiene_dimensiones))
        except Exception:
        ...
        try:
            test_evidencia = {"indicadores": ["Test"], "responsables": ["Test"]}
            evaluador = IndustrialDecatalogoEvaluatorFull()
            analisis = evaluador._analizar_evidencia(test_evidencia)
            validaciones.append(("Análisis de evidencia", analisis is not None))
        except Exception:
        ...
        try:
            evaluador = IndustrialDecatalogoEvaluatorFull()
            ponderacion_correcta = sum(evaluador.ponderacion_dimensiones.values()) == 1.0
            umbrales_correctos = all(
                k in evaluador.umbrales
                for k in ["optimo", "satisfactorio", "basico", "insuficiente"]
            )
            validaciones.append(("Configuración parámetros", ponderacion_correcta and umbrales_correctos))
        except Exception:
        ...
        if todas_validas:
            ...
            print("   El evaluador está correctamente configurado para procesar los 117 planes")
        else:
    ...
    class OrquestadorEvaluacion117:
        ...
        def __init__(self, directorio_planes: str = "planes_desarrollo"):
            ...
            LOGGER.info(f"📁 Orquestador inicializado. Directorio: {self.directorio}")
        ...
        def procesar_todos_los_planes(
            ...
            for idx, archivo_plan in enumerate(archivos_planes, 1):
                try:
                    ...
                    nombre_municipio = archivo_plan.stem.replace('_', ' ').title()
                    ...
                    if exportar_individual:
                        self._exportar_reporte_individual(reporte, nombre_municipio)

                    print(f"   ✅ Completado: {reporte.resumen_ejecutivo['puntaje_global']:.1f}/100")
                ...
                if planes_procesados % 10 == 0:
                    self._mostrar_progreso(planes_procesados, total_planes, planes_exitosos)
            ...
            tiempo_total = (datetime.now() - self.timestamp_inicio).total_seconds()
            ...
            estadisticas = {
                ...
                "tiempo_promedio_segundos": tiempo_total / planes_procesados if planes_procesados > 0 else 0,
                "tasa_exito": planes_exitosos / planes_procesados * 100 if planes_procesados > 0 else 0,
                "puntaje_promedio_global": np.mean(
                    [r.resumen_ejecutivo["puntaje_global"] for r in self.resultados]) if self.resultados else 0,
        ...
        def _exportar_reporte_individual(self, reporte: ReporteFinalDecatalogo, nombre_municipio: str):
            ...
            try:
                ...
                nombre_archivo = nombre_municipio.replace(' ', '_').replace('/', '_')
        ...
        def _mostrar_progreso(self, procesados: int, total: int, exitosos: int):
            ...
            tiempo_transcurrido = (datetime.now() - self.timestamp_inicio).total_seconds()
            tiempo_restante = (tiempo_transcurrido / procesados) * (total - procesados)

            print(f"\n📊 PROGRESO: {procesados}/{total} ({porcentaje:.1f}%)")
            print(f"   Exitosos: {exitosos} ({tasa_exito:.1f}%)")
            print(f"   Tiempo restante estimado: {tiempo_restante / 60:.1f} minutos")
        ...
        def _mostrar_resumen_final(self, estadisticas: Dict[str, Any]):
            ...
            print(f"   Puntaje promedio: {estadisticas['puntaje_promedio_global']:.1f}/100")
            print(f"   Tiempo total: {estadisticas['tiempo_total_segundos'] / 60:.1f} minutos")
            print(f"   Tiempo promedio por plan: {estadisticas['tiempo_promedio_segundos']:.1f} segundos")

        def _generar_reporte_consolidado(
                self,
                estadisticas: Dict[str, Any],
                df_comparacion: Optional[pd.DataFrame]
        ):
            ...
            try:
                ...
                with open(dir_consolidado / "estadisticas_procesamiento.json", 'w') as f:
                ...
                if df_comparacion is not None:
                    ...
                    self._generar_dashboard_html(df_comparacion, dir_consolidado)
        ...
        def _generar_dashboard_html(self, df: pd.DataFrame, directorio: Path):
            ...
            for idx, row in df.head(20).iterrows():
                clase = "high" if row['Puntaje_Global'] >= 70 else "medium" if row['Puntaje_Global'] >= 50 else "low"
                html += f"""
                        <tr class="{clase}">
                            <td>{row['Ranking']}</td>
                            <td>{row['Municipio']}</td>
    ...
    if __name__ == "__main__":
        ...
        parser.add_argument(
            "--procesar",
            type=str,
            help="Directorio con planes a procesar"
        )
        parser.add_argument(
            "--max-planes",
            type=int,
        ...
        parser.add_argument(
            "--exportar",
            action="store_true",
            help="Exportar reportes individuales"
        )
        ...
        if args.validar:
            ...
            estadisticas = orquestador.procesar_todos_los_planes(
                ...
                exportar_individual=args.exportar
            )
