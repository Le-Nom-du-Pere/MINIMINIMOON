...
class IndustrialDecatalogoEvaluatorFull:
    ...
    def _analizar_evidencia(
        ...
        textos_indicadores = self._extraer_textos(
            evidencia, "indicadores", "metas")
        indicador_scores = [
            self.scorer.calculate_feasibility_score(texto)
            for texto in textos_indicadores
        ]
...
class Evaluador:
    ...
    def evaluar_dimension_de1(
        ...
        valor_q2 = self._clamp(
            0.6 + 0.4 * max_score) if mejor_responsable else 0.0
        evaluaciones.append(
            self._crear_evaluacion(
                "Q2",
                "DE-1",
        ...
        evidencia_q4 = "; ".join(
            filter(
                None, [evidencia_q3, self._formatear_responsable(mejor_responsable)])
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "Q4",
        ...
        mejor_impacto = self._seleccionar_mejor_deteccion(
            target_dets + date_dets)
        valor_q5 = self._clamp(
            max_score if mejor_impacto else (
                max_score * 0.5 if target_dets else 0.0)
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "Q5",
        ...
        puntaje_final = self._clamp(
            puntaje_base * matriz_causal["Factor_Causal"])
    ...
    def evaluar_dimension_de2(
        ...
        cobertura_resultados = 0.5 * (1 if baseline_dets else 0) + 0.5 * (
            1 if target_dets else 0
        )
        evidencia_o2 = " | ".join(
            filter(
                None,
        ...
        valor_t1 = (
            self._clamp(
                0.4 + 0.1 * analisis.recursos) if analisis.recursos else 0.0
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "T1",
        ...
        mejor_t2 = self._seleccionar_mejor_deteccion(
            numerical_dets + timeframe_dets)
        evaluaciones.append(
            self._crear_evaluacion(
                "T2",
                "DE-2",
        ...
        evidencia_plazos = self._formatear_deteccion(mejor_s2) or self._obtener_muestra(
            evidencia, "plazos"
        )
    ...
    def evaluar_dimension_de3(
        ...
        valor_g1 = (
            self._clamp(
                0.5 + 0.1 * analisis.recursos) if analisis.recursos else 0.0
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "G1",
        ...
        valor_g2 = self._clamp(
            0.6 if (analisis.plazos or mejor_plazo) else 0.0)
        evaluaciones.append(
            self._crear_evaluacion(
                "G2",
                "DE-3",
        ...
        valor_a2 = self._clamp(
            0.4 * (1 if analisis.recursos else 0)
            + 0.4 * (1 if mejor_responsable else 0)
            + 0.2 * max_score
        )
        evaluaciones.append(
            self._crear_evaluacion(
        ...
        valor_s1 = self._clamp(
            0.4 * max_score
            + 0.3 * (1 if timeframe_dets else 0)
            + 0.3 * (1 if analisis.plazos else 0)
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "S1",
        ...
        valor_s2 = (
            self._clamp(
                0.5 + 0.1 * analisis.riesgos) if analisis.riesgos else 0.0
        )
        evaluaciones.append(
            self._crear_evaluacion(
                "S2",
    ...
    def evaluar_dimension_de4(
        ...
        for idx, descripcion in enumerate(self.preguntas_de4, start=1):
            ...
            if idx == 1:
                valor = self._clamp(
                    0.6 * analisis.max_score + 0.4 * cobertura_baseline)
                evidencia_texto = self._formatear_deteccion(
                    self._seleccionar_mejor_deteccion(baseline_dets)
                )
            elif idx == 2:
                valor = self._clamp(
                    (cobertura_baseline + cobertura_target + cobertura_time) / 3
                )
                evidencia_texto = " | ".join(
                    filter(
                        ...
                        [
                            self._formatear_deteccion(
                                self._seleccionar_mejor_deteccion(
                                    baseline_dets)
                            ),
                            self._formatear_deteccion(
                                self._seleccionar_mejor_deteccion(target_dets)
                ...
                valor = self._clamp(
                    0.5 * analisis.max_score + 0.5 * cobertura_target)
                evidencia_texto = self._formatear_deteccion(
                    self._seleccionar_mejor_deteccion(target_dets)
                )
            elif idx == 4:
                valor = self._clamp(
                    0.4 * cobertura_responsables
                    + 0.4 * cobertura_recursos
                    + 0.2 * analisis.max_score
                )
                evidencia_texto = "; ".join(
                    filter(
                        None,
                ...
                valor = self._clamp(
                    0.5 * cobertura_time
                    + 0.3 * analisis.max_score
                    + 0.2 * cobertura_numerica
                )
                evidencia_texto = self._formatear_deteccion(
                    self._seleccionar_mejor_deteccion(timeframe_dets)
                ...
                valor = self._clamp(
                    (cobertura_target + cobertura_time + cobertura_numerica) / 3
                )
                evidencia_texto = self._formatear_deteccion(
                    self._seleccionar_mejor_deteccion(
                        target_dets + numerical_dets)
                )
    ...
    def evaluar_cluster_completo(
        ...
        for punto_id in cluster.puntos:
            evaluacion_punto, _ = self.evaluar_punto_completo(
...
def integrar_evaluador_decatalogo(
    ...
    try:
        ...
        evaluacion_punto, analisis = evaluador.evaluar_punto_completo(
            evidencia_dimension, dimension.id
        )
        puntajes_dim = {
            dim_eval.dimension: dim_eval.puntaje_dimension
            for dim_eval in evaluacion_punto.evaluaciones_dimensiones
        }
        ...
        identificabilidad_causal = min(
            1.0, (consistencia_logica + robustez_causal) / 2)
        certeza_probabilistica = (
            analisis.max_score if analisis.indicador_scores else consistencia_logica
        )
        ...
        if not riesgos and factibilidad_operativa < 0.5:
            riesgos.append(
                "Planificación presupuestal con evidencia insuficiente según DE-3."
            )

        evaluacion_causal = EvaluacionCausalIndustrial(
            consistencia_logica=max(0.0, min(1.0, consistencia_logica)),
            identificabilidad_causal=max(
                0.0, min(1.0, identificabilidad_causal)),
            ...
            brechas_criticas=sum(
                1 for score in puntajes_dim.values() if score < 60),
        )
        ...
        if factibilidad_operativa < 0.6:
            recomendaciones.append(
                "Reforzar programación financiera y responsabilidades operativas detectadas."
            )
