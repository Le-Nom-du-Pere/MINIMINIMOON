...
Conexión Garantizada: Se integra PERFECTAMENTE con el sistema industrial principal (Pasted_Text_1758577067172.txt).
...
# En tu sistema principal (Pasted_Text_1758577067172.txt), en el método `evaluar_dimension`,
# puedes integrar este evaluador así:
...
def evaluar_dimension(self, dimension: DimensionDecalogo) -> ResultadoDimensionIndustrial:
    ...
    from decatalogo_evaluator_full import IndustrialDecatalogoEvaluatorFull

    # Suponiendo que tienes el texto completo del PDT en self.loader.textos_originales
    texto_completo_pdt = " ".join(self.loader.textos_originales[:1000])  # Primeras 1000 líneas para evaluación

    # Identificar a qué cluster pertenece esta dimensión (mapeo que debes definir)
    cluster_asociado = self._mapear_dimension_a_cluster(dimension.id)
        ...
        if evaluacion_cluster.puntaje_agregado_cluster < 60:
            for eval_punto in evaluacion_cluster.evaluaciones_puntos:
                if eval_punto.puntaje_agregado_punto < 50:
                    self.logger.warning(f"⚠️  [DECÁLOGO] Punto {eval_punto.punto_id} en dimensión {dimension.id} tiene baja calidad: {eval_punto.puntaje_agregado_punto:.1f}")
    # >>>>>>>> FIN DE LA INTEGRACIÓN <<<<<<<<

    # ... resto de tu lógica de evaluación causal ...
"""
