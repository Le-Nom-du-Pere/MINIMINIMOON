...
class TestErrorClassifier(unittest.TestCase):
    ...
    def test_permission_error_classified_as_transient(self):
        ...
        error_type, specific = classifier.classify_error(PermissionError("Permission denied"))
    ...
    def test_file_not_found_classified_as_permanent(self):
        classifier = ErrorClassifier()
        error_type, specific = classifier.classify_error(FileNotFoundError("File not found"))
    ...
    def test_timeout_error_classified_as_transient(self):
        classifier = ErrorClassifier()
        error_type, specific = classifier.classify_error(TimeoutError("Connection timeout"))
        self.assertEqual(error_type, ErrorType.TRANSIENT)
...
class TestRetryLogic(unittest.TestCase):
    ...
    def test_transient_error_with_retry(self):
        ...
        self.assertGreaterEqual(result.attempts, 3)  # At least initial + 2 retries
        self.assertEqual(result.error.error_type, ErrorType.TRANSIENT)
        self.assertGreaterEqual(elapsed_time, 0.3)  # Should have delays (0.1 + 0.2)
...
class TestFeasibilityPlanProcessor(unittest.TestCase):
    ...
    def test_handles_empty_indicators(self):
        ...
        self.assertFalse(result.success)
        self.assertEqual(result.error.error_type, ErrorType.PERMANENT)
        self.assertEqual(result.attempts, 1)  # Should not retry permanent errors
    ...
    def test_generates_plan_id_when_none_provided(self):
        ...
        self.assertEqual(len(result.plan_id), 8)  # MD5 hash truncated to 8 chars
    ...
    def test_batch_processing(self):
        ...
        plans = [
            ...
            ({"simulate_file_not_found": True, "indicators": ["Indicator 3"]}, "plan_3")
        ]
...
class TestIntegration(unittest.TestCase):
    ...
    def test_complete_error_logging_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            processor = FeasibilityPlanProcessor(retry_config=config, log_directory=temp_dir)
            ...
            with open(log_files[0], 'r') as f:
                ...
                self.assertTrue("attempt number: 2" in content.lower() or "attempt_number" in content.lower())
...
class TestErrorScenarios(unittest.TestCase):
    ...
    def test_all_simulated_errors(self):
        ...
        error_scenarios = [
            ("simulate_permission_error", ErrorType.TRANSIENT, TransientErrorType.FILE_PERMISSION),
            ("simulate_file_not_found", ErrorType.PERMANENT, PermanentErrorType.FILE_NOT_FOUND),
            ("simulate_network_timeout", ErrorType.TRANSIENT, TransientErrorType.NETWORK_TIMEOUT),
            ("simulate_malformed_pdf", ErrorType.PERMANENT, PermanentErrorType.MALFORMED_PDF),
            ("simulate_memory_error", ErrorType.PERMANENT, PermanentErrorType.OUT_OF_MEMORY),
        ]
    ...
    def test_concurrent_processing_safety(self):
        ...
        def process_plans(processor_id):
            processor = FeasibilityPlanProcessor(log_directory=f"test_logs_{processor_id}")
        ...
        for i in range(3):
            thread = threading.Thread(target=lambda i=i: results.update({i: process_plans(i)}))
        ...
        for i in range(3):
            ...
            self.assertIn(f"processor {i}", results[i].result_data["indicator_results"][0]["indicator"])
