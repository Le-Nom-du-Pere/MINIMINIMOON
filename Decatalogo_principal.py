...
# Paralelismo
from joblib import (  # noqa: F401  (se mantiene para compatibilidad futura)
    Parallel,
    delayed,
)
...
if not SENTENCE_TRANSFORMERS_AVAILABLE:
    raise SystemExit(
        "sentence-transformers no disponible. Instale: pip install sentence-transformers"
    )
...
class TeoriaCambio:
    ...
    def verificar_identificabilidad(self) -> bool:
        return bool(
            self.supuestos_causales and self.mediadores and self.resultados_intermedios
        )
...
class DimensionDecalogo:
    ...
    def evaluar_coherencia_causal(self) -> float:
        ...
        coherencia += (
            0.3
            if any(e.tipo == TipoCadenaValor.IMPACTOS for e in self.eslabones)
            else 0.0
        )
        peso_total += 0.3
    ...
    def calcular_kpi_global(self) -> float:
        return (
            (sum(e.kpi_ponderacion for e in self.eslabones) / len(self.eslabones))
            if self.eslabones
            else 0.0
        )
    ...
    def generar_matriz_riesgos(self) -> Dict[str, List[str]]:
        ...
        for e in self.eslabones:
            riesgos = []
            if not e.indicadores:
                riesgos.append("Falta de indicadores de desempeño")
            if e.ventana_temporal[1] - e.ventana_temporal[0] > 24:
                riesgos.append("Ventana temporal excesivamente amplia")
            if len(e.capacidades_requeridas) < 2:
...
def cargar_decalogo_industrial() -> List[Any]:
    ...
    if json_path.exists():
        try:
            ...
            for i, item in enumerate(data):
                if not all(
                    k in item
                    for k in ["id", "nombre", "cluster", "teoria_cambio", "eslabones"]
                ):
    ...
    for dim_id in range(1, 11):
        dim = {
            ...
            "teoria_cambio": {
                "supuestos_causales": [
                    f"Supuesto causal 1 ({dim_id})",
                    f"Supuesto causal 2 ({dim_id})",
                ],
                "mediadores": {
                    "institucionales": [
                        f"mediador_institucional_{dim_id}_1",
                        f"mediador_institucional_{dim_id}_2",
                    ],
                    "comunitarios": [
                        f"mediador_comunitario_{dim_id}_1",
                        f"mediador_comunitario_{dim_id}_2",
                    ],
                },
                ...
                "precondiciones": [
                    f"precondicion_{dim_id}_1",
                    f"precondicion_{dim_id}_2",
                ],
            },
            "eslabones": [],
        }
        for tipo_idx, tipo in enumerate(
            ...
            dim["eslabones"].append(
                {
                    ...
                    "tipo": tipo,
                    "indicadores": [
                        f"indicador_{tipo.lower()}_{dim_id}_{i + 1}" for i in range(3)
                    ],
                    "capacidades_requeridas": [
                        f"capacidad_{tipo.lower()}_{dim_id}_{i + 1}" for i in range(2)
                    ],
                    "puntos_criticos": [
                        f"punto_critico_{tipo.lower()}_{dim_id}_{i + 1}"
                        for i in range(2)
                    ],
...
def obtener_decalogo_contexto() -> DecalogoContext:
    ...
    for cluster_id, data in _CLUSTER_DEFINITIONS.items():
        ...
        nombre_dimension = next(
            (dimensiones_por_id[p].cluster for p in puntos if p in dimensiones_por_id),
            data["titulo"],
        )
    ...
    _DECALOGO_CONTEXT_CACHE = DecalogoContext(
        dimensiones_por_id, clusters_por_id, cluster_por_dimension
    )
    return _DECALOGO_CONTEXT_CACHE
...
class ExtractorEvidenciaIndustrial:
    ...
    def _precomputar_embeddings(self):
        ...
        if textos_validos:
            try:
                ...
                self.logger.info(
                    f"✅ Embeddings precomputados: {len(textos_validos)} segmentos - {self.nombre_plan}"
                )
            except Exception as e:
            ...
            self.embeddings_doc = torch.tensor([])
            self.logger.warning(
                f"⚠️ Textos insuficientes para embeddings - {self.nombre_plan}"
            )
    ...
    def _build_result(
        self, texto: str, pagina: int, sim: float, rel: float, dens: float
    ) -> Dict[str, Any]:
        return {
            "texto": texto,
            "pagina": pagina,
    ...
    def buscar_evidencia_causal(
        ...
        if (self.embeddings_doc is None) or (self.embeddings_doc.numel() == 0):
            self.logger.warning(
                "⚠️ Embeddings no disponibles. Se usa fallback en tiempo real."
            )
        ...
        try:
            ...
            indices_top = torch.topk(
                sims, min(top_k * 2, len(self.textos_originales))
            ).indices
            for idx in indices_top:
                ...
                if pagina is None:
                    continue
                coincidencias = sum(
                    1 for c in conceptos_clave if c.lower() in texto.lower()
                )
    ...
    def buscar_segmentos_semanticos_global(
        ...
        try:
                    ...
                    for doc_idx, s in enumerate(sims):
                        ...
                        if doc_idx < len(self.textos_originales):
                            ...
                            if len(heap_global) < max_segmentos:
                                heapq.heappush(
                                    heap_global, (score, doc_idx, qi_global, datos)
                                )
            ...
            self.logger.info(
                f"✅ Búsqueda global: {len(resultados)} segmentos seleccionados"
            )
            return resultados
        except Exception as e:
    ...
    def extraer_variables_operativas(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Extracción dimensión {dimension.id}: {sum(len(v) for v in variables.values())} variables"
            )
        except Exception as e:
            self.logger.error(
                f"❌ Error extrayendo variables (dim {dimension.id}): {e}"
            )
        return variables
...
class PDFLoaderIndustrial:
    def __init__(self, file_path: Path):
        ...
        if not PDFPLUMBER_AVAILABLE:
            raise RuntimeError(
                "❌ pdfplumber no está instalado. Instale: pip install pdfplumber"
            )
...
class SistemaEvaluacionIndustrial:
    ...
    def _identificar_brechas_industrial(
        ...
        for e in dimension.eslabones:
            tiene = any(
                ind.lower() in ev.get("texto", "").lower()
                for ind in e.indicadores
                for ev in evidencia.get("indicadores", [])
            )
            if not tiene:
                brechas.append(
                    f"🔴 BRECHA CRÍTICA: Falta especificación de indicadores en {e.id} ({e.tipo.value})"
                )
        if not evidencia.get("recursos", []):
            brechas.append(
                "🔴 BRECHA CRÍTICA: No se encontró especificación presupuestal/recursos"
            )
    ...
    def _generar_recomendaciones_industrial(
        self,
        dimension: DimensionDecalogo,
        ev: EvaluacionCausalIndustrial,
        brechas: List[str],
    ) -> List[str]:
        recs: List[str] = []
        if ev.consistencia_logica < 0.7:
            recs.append("🔧 FORTALECER coherencia lógica de la teoría de cambio")
        if ev.factibilidad_operativa < 0.6:
            recs.append("🔧 FORTALECER mecanismos de implementación")
        if ev.certeza_probabilistica < 0.7:
            recs.append("📊 EVIDENCIA: Incrementar soporte empírico")
        if ev.robustez_causal < 0.6:
            recs.append("🧩 SIMPLIFICAR el modelo causal")
        for b in brechas:
            if "BRECHA CRÍTICA" in b:
                recs.append(f"🚨 ACCIÓN INMEDIATA: {b.split(': ', 1)[1]}")
            elif "BRECHA IMPORTANTE" in b:
                recs.append(f"⚠️ PRIORIDAD ALTA: {b.split(': ', 1)[1]}")
            elif "BRECHA MODERADA" in b:
                recs.append(f"🔧 MEJORA CONTINUA: {b.split(': ', 1)[1]}")
        if ev.evidencia_soporte < 5:
            recs.append("📚 INVESTIGACIÓN: Levantar línea base robusta")
        if len(ev.riesgos_implementacion) > 3:
            recs.append("🛡️ GESTIÓN: Plan integral de mitigación de riesgos")
    ...
    def _evaluar_coherencia_causal_industrial(
        ...
        try:
                ...
                try:
                    if len(G.nodes) > 2:
                        ...
                        if nm:
                            nodos.extend(
                                np.random.choice(
                                    nm, size=min(3, len(nm)), replace=False
                                ).tolist()
                            )
                        if nr:
                            nodos.extend(
                                np.random.choice(
                                    nr, size=min(2, len(nr)), replace=False
                                ).tolist()
                            )
                        sub = G.subgraph(nodos)
                        if (
                            nx.is_directed_acyclic_graph(sub)
                            and nx.has_path(sub, "insumos", "impactos")
                            and len(sub.edges) >= 2
                        ):
                            certezas.append(1.0)
                        else:
            ...
            if evidencia_soporte == 0:
                ...
                factibilidad *= 0.5
            if certeza < 0.7:
                riesgos.append("⚠️ Baja certeza causal: fortalecer marco teórico")
            if robustez < 0.6:
                riesgos.append("⚠️ Baja robustez causal: simplificar relaciones")
    ...
    def evaluar_dimension(
        ...
        try:
            self.logger.info(
                f"🔍 Evaluando dimensión {dimension.id}: {dimension.nombre}"
            )
            ...
            try:
                from Decatalogo_evaluador import (  # noqa: F401
                    integrar_evaluador_decatalogo,
                )
            ...
            self.logger.info(
                f"✅ Resultado dimensión {dimension.id}: {resultado.puntaje_final:.1f}/100"
            )
            return resultado
        except Exception as e:
            ...
            return ResultadoDimensionIndustrial(
                ...
                evidencia={},
                brechas_identificadas=[f"Error crítico: {str(e)}"],
                recomendaciones=["Revisión manual urgente"],
                timestamp_evaluacion=datetime.now().isoformat(),
            )
    ...
    def _generar_recomendacion_global(
        ...
        total = len(puntajes)
        if promedio >= 85:
            return "IMPLEMENTACIÓN INTEGRAL RECOMENDADA"
        if promedio >= 70 and deficientes <= total * 0.3:
            return "IMPLEMENTACIÓN SELECTIVA RECOMENDADA"
        if promedio >= 50:
            return "REDISEÑO PARCIAL REQUERIDO"
        ...
        try:
            ...
            analisis_agregado = {
                ...
                "certidumbre_predominante": (
                    max(set(certs), key=certs.count) if certs else "N/A"
                ),
...
class GeneradorReporteIndustrial:
    ...
    def generar_reporte_markdown(
        ...
        for r in resultados:
            ...
            rep.append(
                f"### 💡 Recomendación Estratégica: {r.evaluacion_causal.recomendacion_estrategica}"
            )
            rep.append("")
            ...
            if evidencia_total > 0:
                ...
                for tipo, evids in r.evidencia.items():
                    ...
                    for e in evids[:3]:
                        frag = (
                            (e.get("texto", "")[:140] + "...") if e.get("texto") else ""
                        )
                        rep.append(
                            f"  - p.{e.get('pagina', '?')} — {frag} (score={e.get('score_final', 0):.2f})"
                        )
                rep.append("")
        return "\n".join(rep)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluación Industrial de Planes de Desarrollo Municipal"
    )
    parser.add_argument("--pdf", required=True, type=str, help="Ruta al PDF del Plan")
    parser.add_argument(
        "--salida_json",
        type=str,
        default="reporte_industrial.json",
        help="Archivo JSON de salida",
    )
    parser.add_argument(
        "--salida_md",
        type=str,
        default="reporte_industrial.md",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Número de dimensiones a procesar (1-10)",
    )
    ...
    dimensiones = [
        contexto.dimensiones_por_id[i] for i in range(1, min(10, args.top_dim) + 1)
    ]
    resultados: List[ResultadoDimensionIndustrial] = []
    ...
    try:
        md = GeneradorReporteIndustrial.generar_reporte_markdown(
            resultados,
            sistema.pdf_path.stem,
