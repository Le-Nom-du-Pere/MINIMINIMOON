...
class ResponsibilityDetector:
    ...
    def __init__(self, model_name: str = "es_core_news_sm"):
        ...
        self.compiled_gov_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.government_patterns
        ]
        self.compiled_pos_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.position_patterns
        ]
        ...
        for ent in doc.ents:
            if ent.label_ in ["PERSON", "PER"]:
                ...
                entity = ResponsibilityEntity(
                    ...
                    confidence=(
                        0.8 if is_gov else 0.6
                    ),  # Higher confidence for government entities
                    start_pos=ent.start_char,
                    end_pos=ent.end_char,
        ...
        for next_entity in entities[1:]:
            ...
            if next_entity.start_pos <= current.end_pos:
                ...
                # If same confidence, prefer government entities
                    ...
                    and next_entity.is_government
                    and not current.is_government
                ):
                    current = next_entity
            else:
