...
def test_scoring():
    ...
    for test_case in test_texts:
        ...
        print(
            f"  - Similarity Score: {result['similarity_score']:.2f} (w1={result['weights']['w1']})"
        )
        print(
            f"  - Causal Density: {result['causal_density']:.6f} (w2={result['weights']['w2']})"
        )
        print(
            f"  - Informative Ratio: {result['informative_length_ratio']:.3f} (w3={result['weights']['w3']})"
        )
...
def test_weight_validation():
    ...
    try:
        ...
        print(
            f"✓ Weight update successful: w1={scorer.w1}, w2={scorer.w2}, w3={scorer.w3}"
        )
    except ValueError as e:
