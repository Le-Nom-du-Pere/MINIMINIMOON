        ...
        for i in range(0, len(pairs), batch_size):
            ...
            for j, prob_dist in enumerate(probs):
                ...
                results.append({
                    ...
                    "is_contradiction": predicted_label == "contradiction",
                    "contradiction_score": float(prob_dist[2])  # Index 2 is contradiction
                })
...
class CompetenceValidator:
    ...
    def _get_default_matrix(self) -> CompetenceMatrix:
        ...
        return CompetenceMatrix(
            municipal={
                "salud": [
                    "gestionar", "coordinar", "articular", "promover",
                    "vigilar", "supervisar", "cofinanciar", "apoyar"
                ],
                "educacion": [
                    "mantener infraestructura", "dotar", "gestionar",
                    "promover acceso", "coordinar", "apoyar"
                ],
                "agua": [
                    "prestar servicio", "garantizar acceso", "mantener",
                    "construir acueducto local", "supervisar"
                ],
                "ambiente": [
                    "proteger recursos locales", "gestionar residuos",
                    "controlar emisiones locales", "educar"
                ],
                "movilidad": [
                    "mantener vías terciarias", "señalizar", "organizar",
                    "regular transporte local", "construir andenes"
                ],
                "seguridad": [
                    "apoyar", "coordinar", "prevenir", "promover convivencia",
                    "gestionar fondos seguridad"
                ],
                "cultura": [
                    "promover", "apoyar", "gestionar espacios", "fomentar",
                    "preservar patrimonio local"
                ],
                "vivienda": [
                    "promover", "gestionar subsidios", "titular predios",
                    "mejorar entorno", "legalizar asentamientos"
                ]
            },
            departmental={
                "salud": [
                    "administrar red hospitalaria", "contratar especialistas",
                    "dirigir ESE nivel 2-3", "coordinar referencia"
                ],
                "educacion": [
                    "nombrar docentes", "administrar nómina", "evaluar",
                    "certificar instituciones", "definir plantas"
                ],
                "agua": [
                    "planificar regional", "asistir técnicamente",
                    "coordinar intermunicipal", "grandes obras"
                ],
                "ambiente": [
                    "otorgar licencias regionales", "administrar parques",
                    "controlar cuencas", "regular regional"
                ],
                "movilidad": [
                    "construir vías secundarias", "mantener red departamental",
                    "regular intermunicipal", "grandes terminales"
                ],
                "seguridad": [
                    "coordinar regional", "apoyar policía departamental",
                    "gestionar cárceles departamentales"
                ]
            },
            national={
                "salud": [
                    "definir política", "regular sistema", "administrar FOSYGA",
                    "habilitar IPS", "formar especialistas"
                ],
                "educacion": [
                    "definir currículo nacional", "expedir títulos superiores",
                    "regular profesiones", "administrar ICFES"
                ],
                "defensa": [
                    "comandar fuerzas", "combatir narcotráfico",
                    "defender soberanía", "inteligencia nacional"
                ],
                "justicia": [
                    "nombrar jueces", "administrar justicia",
                    "legislar", "indultar"
                ],
                "relaciones_exteriores": [
                    "representar país", "negociar tratados",
                    "política exterior", "servicio diplomático"
                ]
            },
            shared={
                "salud": {
            ...
            legal_basis={
                "constitucion": ["Art. 311", "Art. 313", "Art. 287", "Art. 356", "Art. 357"],
                "leyes": ["Ley 136/1994", "Ley 715/2001", "Ley 1551/2012", "Ley 152/1994"],
            ...
            if sector in self.overreach_patterns:
                ...
                for match in pattern.finditer(text):
                    ...
                    issue = {
                        ...
                        "context": context,
                        "required_level": self._get_required_level(match.group(), sector),
                        "legal_basis": self._get_legal_basis(sector),
                        "explanation": self._explain_overreach(match.group(), sector, level),
    ...
    def _get_required_level(self, action: str, sector: str) -> str:
        ...
        if any(word in action_lower for word in 
               ["hospital", "ESE nivel 2", "ESE nivel 3", "red hospitalaria"]):
            return "departmental"
        
        if any(word in action_lower for word in
               ["docentes", "profesores", "nómina educativa", "plantas docentes"]):
            return "departmental"
        
        if any(word in action_lower for word in
               ["ejército", "fuerzas armadas", "policía nacional", "narcotráfico"]):
            return "national"
    ...
    def _suggest_fix(self, action: str, sector: str) -> str:
        ...
        return suggestions.get(sector, "Considere reformular usando verbos de coordinación y apoyo.")
...
class RiskScorer:
    ...
    def calculate_risk(
        ...
        empirical_coverage = self._calculate_coverage() if self.calibration_data else 0.9
    ...
    def _score_contradictions(self, contradictions: List[Any]) -> float:
        ...
        if scores:
            ...
            count_factor = min(1.0, len(scores) / 10)  # Normalize by expected count
    ...
    def _train_conformal_predictor(self):
        ...
        if len(self.calibration_data) < 30:
            logger.warning("Insufficient calibration data for conformal prediction")
            return
        
        try:
            # Prepare training data
            X = np.array([[d['contra'], d['comp'], d['agenda']] 
