...
class CausalPatternDetector:
    ...
    def _calculate_context_adjusted_confidence(
        self, base_confidence: float, context: str, connector: str
    ) -> float:
        """
        ...
        if connector in ["implica"]:
            ...
            math_indicators = ["ecuación", "fórmula",
                               "teorema", "lógica", "matemática"]
            ...
            if any(
                indicator in context.lower() for indicator in instrumental_indicators
            ):
    ...
    def _resolve_overlapping_matches(
        ...
        for match in matches:
            ...
            overlaps = any(
                self._matches_overlap(match, existing) for existing in filtered_matches
            )
    ...
    def calculate_pattern_statistics(self, text: str) -> Dict[str, any]:
        ...
        pattern_types = {}
        for match in matches:
            pattern_types[match.pattern_type] = (
                pattern_types.get(match.pattern_type, 0) + 1
            )
