...
def _read_config(path: Path = CONFIG_PATH) -> Dict[str, object]:
    if not path.exists():
        raise DecalogoProviderError(f"No existe configuración de decálogo en {path}")
...
def provide_decalogos() -> Dict[str, object]:
    ...
    if not config.get("autoload", False):
        raise DecalogoProviderError("El autoload está deshabilitado en la configuración")
    ...
    crosswalk_path = Path(config.get("crosswalk", bundle_dir / "crosswalk.latest.json"))

    for path in [full_path, industrial_path, dnp_path, crosswalk_path]:
        if not Path(path).exists():
            raise DecalogoProviderError(f"No se encuentra el archivo requerido: {path}")

    bundle = load_decalogos(
        [str(full_path), str(industrial_path), str(dnp_path)], crosswalk_path=str(crosswalk_path)
    )
    return bundle
