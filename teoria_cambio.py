...
class TeoriaCambio:
            ...
            for sucesor in grafo.successors(nodo):
                ...
                if not self._es_conexion_valida(categoria_nodo, categoria_sucesor):
                    violacion = {
                        'origen': nodo,
                        'destino': sucesor,
                        'categoria_origen': categoria_nodo.name if categoria_nodo else 'INDEFINIDA',
                        'categoria_destino': categoria_sucesor.name if categoria_sucesor else 'INDEFINIDA'
                    }
    ...
    def detectar_caminos_completos(self, grafo: nx.DiGraph = None) -> ValidacionResultado:
        """
        ...
        nodos_insumos = self._obtener_nodos_por_categoria(grafo, CategoriaCausal.INSUMOS)
        nodos_impactos = self._obtener_nodos_por_categoria(grafo, CategoriaCausal.IMPACTOS)
    ...
    def _generar_sugerencias_conexiones(self, grafo: nx.DiGraph, resultado: ValidacionResultado):
        """
        Genera sugerencias específicas para conectar categorías faltantes
        """
        nodos_por_categoria = {}
        for categoria in CategoriaCausal:
            nodos_por_categoria[categoria] = self._obtener_nodos_por_categoria(grafo, categoria)
