...
def ejemplo_teoria_cambio_completa():
    ...
    print(f"Categorías faltantes: {[cat.name for cat in resultado.categorias_faltantes]}")
...
def ejemplo_teoria_cambio_problematica():
    ...
    if resultado.violaciones_orden:
        for violacion in resultado.violaciones_orden:
            print(f"  - Conexión inválida: {violacion['categoria_origen']} → {violacion['categoria_destino']}")
...
def ejemplo_analisis_detallado():
    ...
    print(f"Categorías faltantes: {[cat.name for cat in sugerencias.categorias_faltantes]}")
...
def ejemplo_validacion_individual():
    ...
    for nodo, categoria_esperada in nodos_test:
        ...
        print(f"{nodo}: {categoria_detectada.name} {'✓' if categoria_detectada == categoria_esperada else '✗'}")
