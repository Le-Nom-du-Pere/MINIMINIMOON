...
def test_autoload_bundle() -> None:
    ...
    assert "from pdm_contra.bridges.decatalogo_provider import provide_decalogos" in principal
    assert "BUNDLE = provide_decalogos()" in principal
    assert "from pdm_contra.bridges.decatalogo_provider import provide_decalogos" in evaluador
