...
class DocumentSegmenter:
    ...
    def _segment_with_spacy(self, text: str) -> List[Dict[str, Any]]:
        ...
        while i < len(sentences):
            ...
            projected_char_count = current_char_count + sent_char_count + 1  # +1 for space
            current_sent_count = len(current_segment_sents)
            
            # Primary criterion: sentence count (with flexibility)
            target_sentences_met = (current_sent_count >= self.target_sentences - self.max_sentence_deviation and
                                  current_sent_count <= self.target_sentences + self.max_sentence_deviation)
            
            # Secondary criterion: character range
            in_char_range = (self.target_char_min <= projected_char_count <= self.target_char_max)
            
            # Decision matrix for dual criteria
            should_finalize_segment = self._should_finalize_segment(
                current_sent_count, projected_char_count, sent_char_count, i, len(sentences)
            )
            
            if should_finalize_segment:
                # Finalize current segment
                segment_text = " ".join(current_segment_sents)
                segments.append(self._create_segment_dict(segment_text, current_segment_sents, "sentence_based"))
    ...
    def _should_finalize_segment(self, current_sent_count: int, projected_char_count: int, 
                               next_sent_char_count: int, sent_index: int, total_sentences: int) -> bool:
        """
        Intelligent decision logic for segment finalization using dual criteria.
        """
        current_char_count = projected_char_count - next_sent_char_count - 1  # Approximate current count
        ...
        if (current_sent_count >= self.target_sentences - self.max_sentence_deviation and 
            next_sent_char_count > 400):  # Very long sentence
            return True
            
        # If we're near the end and have reasonable content
        if (sent_index >= total_sentences - 2 and 
            current_sent_count >= 2 and 
    ...
    def _segment_with_rules(self, text: str) -> List[Dict[str, Any]]:
        ...
        sentences = [s.strip() for s in potential_sentences if s.strip() and len(s.strip()) > 10]
        ...
        for i, sent in enumerate(sentences):
            ...
            if should_finalize:
                segment_text = " ".join(current_segment_sents)
                segments.append(self._create_segment_dict(segment_text, current_segment_sents, "rule_based"))
    ...
    def _character_based_segmentation(self, text: str) -> List[Dict[str, Any]]:
        ...
        current_char_count = 0
        target_chars = (self.target_char_min + self.target_char_max) // 2  # Use midpoint
        
        for word in words:
            word_length = len(word)
            projected_length = current_char_count + word_length + len(current_segment_words)  # +spaces
            ...
            if (projected_length > target_chars and 
                ...
                segments.append(self._create_segment_dict(segment_text, [], "character_based"))
    ...
    def _create_segment_dict(self, text: str, sentences: List[str], segment_type: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'sentences': sentences,
            'metrics': metrics,
            'meets_char_criteria': self.target_char_min <= char_count <= self.target_char_max,
            'meets_sentence_criteria': abs(sentence_count - self.target_sentences) <= self.max_sentence_deviation,
            'segment_type': segment_type
        }
    ...
    def _estimate_semantic_coherence(self, text: str) -> float:
        ...
        if self.nlp:
            try:
                doc = self.nlp(text)
                sent_lengths = [len(sent.text) for sent in doc.sents if sent.text.strip()]
                if len(sent_lengths) > 1:
                    length_variance = statistics.variance(sent_lengths) / max(statistics.mean(sent_lengths), 1)
        ...
        for i, segment in enumerate(segments):
            ...
            if metrics.char_count < self.min_segment_chars and len(segments) > 1:
                if processed_segments:
                    ...
                    original_type = prev_segment.get('segment_type', segment.get('segment_type', 'merged'))
    ...
    def _split_large_segment(self, segment: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if sentences and len(sentences) > 1:
            ...
            return [
                self._create_segment_dict(" ".join(first_half), first_half, "split_large"),
                self._create_segment_dict(" ".join(second_half), second_half, "split_large")
            ]
        else:
    ...
    def _calculate_segmentation_stats(self, segments: List[Dict[str, Any]]):
        ...
        self.segmentation_stats.char_length_distribution = self._create_char_distribution(char_lengths)
    ...
    def _emergency_fallback_segmentation(self, text: str) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(text), target_size):
            ...
            if chunk:
                segments.append(self._create_segment_dict(chunk, [], "emergency_fallback"))
    ...
    def get_segmentation_report(self) -> Dict[str, Any]:
        ...
        char_range_success_rate = (stats.segments_in_char_range / stats.total_segments) * 100
        sentence_target_success_rate = (stats.segments_with_3_sentences / stats.total_segments) * 100
        ...
        sentence_std_dev = statistics.stdev(sentence_counts) if len(sentence_counts) > 1 else 0
    ...
    def _calculate_overall_quality_score(self) -> float:
        ...
        coherence_scores = [seg.semantic_coherence_score for seg in self.segmentation_stats.segments]
    ...
    def log_segmentation_metrics(self):
        ...
        logger.info(f"Average sentence count: {report['summary']['avg_sentence_count']}")
        logger.info(f"Character range success rate: {report['summary']['char_range_success_rate']}%")
        logger.info(f"Sentence target success rate: {report['summary']['sentence_target_success_rate']}%")
        logger.info(f"Overall quality score: {report['quality_indicators']['overall_quality_score']:.3f}")
