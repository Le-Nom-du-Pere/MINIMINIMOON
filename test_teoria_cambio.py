...
class TestTeoriaCambioValidacion(unittest.TestCase):
    ...
    def test_generar_sugerencias_categorias_faltantes(self):
        ...
        categorias_faltantes_nombres = [cat.name for cat in resultado.categorias_faltantes]
    ...
    def test_obtener_categoria_nodo_por_nombre(self):
        ...
        assert tc._obtener_categoria_nodo("recursos_insumos", grafo) == CategoriaCausal.INSUMOS
        assert tc._obtener_categoria_nodo("actividad_proceso", grafo) == CategoriaCausal.PROCESOS
        assert tc._obtener_categoria_nodo("entregable_producto", grafo) == CategoriaCausal.PRODUCTOS
        assert tc._obtener_categoria_nodo("cambio_resultado", grafo) == CategoriaCausal.RESULTADOS
        assert tc._obtener_categoria_nodo("transformacion_impacto", grafo) == CategoriaCausal.IMPACTOS
    ...
    def test_obtener_categoria_nodo_por_posicion(self):
        ...
        assert tc._obtener_categoria_nodo("nodo_inicial", grafo) == CategoriaCausal.INSUMOS
        assert tc._obtener_categoria_nodo("nodo_final", grafo) == CategoriaCausal.IMPACTOS
        assert tc._obtener_categoria_nodo("nodo_medio", grafo) == CategoriaCausal.PRODUCTOS
    ...
    def test_es_conexion_valida(self):
        ...
        assert tc._es_conexion_valida(CategoriaCausal.INSUMOS, CategoriaCausal.PROCESOS)  # diff = 1
        assert tc._es_conexion_valida(CategoriaCausal.INSUMOS, CategoriaCausal.PRODUCTOS)  # diff = 2
        assert tc._es_conexion_valida(CategoriaCausal.PROCESOS, CategoriaCausal.RESULTADOS)  # diff = 2
        
        # Conexiones inválidas (diferencia > 2 o <= 0)
        assert not tc._es_conexion_valida(CategoriaCausal.INSUMOS, CategoriaCausal.RESULTADOS)  # diff = 3
        assert not tc._es_conexion_valida(CategoriaCausal.INSUMOS, CategoriaCausal.IMPACTOS)  # diff = 4
        assert not tc._es_conexion_valida(CategoriaCausal.PROCESOS, CategoriaCausal.INSUMOS)  # diff = -1
    ...
    def test_es_camino_completo(self):
        ...
        nodos = ["insumos", "proceso_actividad", "producto_entregable", "resultado_cambio", "impactos"]
        for nodo in nodos:
            grafo.add_node(nodo)
        
        # Camino completo
        camino_completo = ["insumos", "proceso_actividad", "producto_entregable", "resultado_cambio", "impactos"]
    ...
    def test_obtener_nodos_por_categoria(self):
        ...
        nodos_procesos = tc._obtener_nodos_por_categoria(grafo, CategoriaCausal.PROCESOS)
        nodos_impactos = tc._obtener_nodos_por_categoria(grafo, CategoriaCausal.IMPACTOS)
