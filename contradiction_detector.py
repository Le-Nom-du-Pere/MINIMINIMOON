...
class ContradictionDetector:
    ...
    def __init__(self, context_window: int = 150):
        ...
        self.compiled_adversative = [re.compile(p, re.IGNORECASE | re.UNICODE) for p in self.adversative_patterns]
        self.compiled_goals = [re.compile(p, re.IGNORECASE | re.UNICODE) for p in self.goal_patterns]
    ...
    def _calculate_contradiction_confidence(adversative_pos: int,
        ...
        match_types = sum([bool(goal_matches), bool(action_matches), bool(quantitative_matches)])
    ...
    def detect_contradictions(self, text: str) -> ContradictionAnalysis:
        ...
        adversative_matches = self._find_pattern_matches(normalized_text, self.compiled_adversative, "adversative")
        ...
        for adversative_text, adv_start, adv_end in adversative_matches:
            ...
            goal_matches = self._find_pattern_matches(context, self.compiled_goals, "goal")
            action_matches = self._find_pattern_matches(context, self.compiled_actions, "action")
            quantitative_matches = self._find_pattern_matches(context, self.compiled_quantitative, "quantitative")
    ...
    def integrate_with_risk_assessment(self, text: str, existing_score: float = 0.0) -> Dict[str, float]:
        ...
        integrated_score = existing_score + (contradiction_risk * 0.3)  # 30% weight for contradictions
        ...
        return {
            ...
            "highest_contradiction_confidence": contradiction_analysis.highest_confidence_contradiction.confidence if contradiction_analysis.highest_confidence_contradiction else 0.0
        }
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        if analysis.contradictions:
            ...
            for j, contradiction in enumerate(analysis.contradictions, 1):
                print(f"  {j}. Conector adversativo: '{contradiction.adversative_connector}'")
                print(f"     Palabras clave de meta: {contradiction.goal_keywords}")
                print(f"     Verbos de acción: {contradiction.action_verbs}")
                print(f"     Objetivos cuantitativos: {contradiction.quantitative_targets}")
        ...
        risk_integration = detector.integrate_with_risk_assessment(text, existing_score=0.2)
        print(f"INTEGRACIÓN CON SISTEMA DE RIESGO:")
        print(f"  Puntuación base: {risk_integration['base_score']:.3f}")
        print(f"  Riesgo por contradicciones: {risk_integration['contradiction_risk']:.3f}")
