...
class MemoryWatchdog:
    ...
    def __init__(self, memory_limit_mb: Optional[int] = None, check_interval: float = 1.0):
        """
    ...
    def _get_memory_limit(self, memory_limit_mb: Optional[int]) -> int:
        ...
        if env_limit:
            try:
                ...
                # Create a temporary logger for this specific case since self.logger isn't set up yet
                temp_logger = logging.getLogger(__name__)
                temp_logger.warning(f"Invalid MEMORY_LIMIT_MB value: {env_limit}, using default")
        
        # Conservative default: 2GB per worker process
        return 2048
    
    def _setup_logging(self):
        """Setup logging for memory watchdog events."""
        if not self.logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                '%(asctime)s - MemoryWatchdog - %(levelname)s - %(message)s'
            )
    ...
    def register_process(self, pid: int) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Registered process {pid} for memory monitoring (limit: {self.memory_limit_mb}MB)")
            return True
    ...
    def start_monitoring(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Started memory monitoring (limit: {self.memory_limit_mb}MB, "
    ...
    def stop_monitoring(self, timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            if self._monitoring_thread.is_alive():
                self.logger.warning(f"Memory monitoring thread did not stop within {timeout}s timeout")
                return False
            else:
            ...
            try:
                ...
                if memory_usage.rss_mb > self.memory_limit_mb:
                    ...
                    if memory_usage.rss_mb > self.memory_limit_mb * 0.8:
                        self.logger.warning(f"Process {pid} high memory usage: "
                                          f"{memory_usage.rss_mb:.1f}MB (limit: {self.memory_limit_mb}MB)")
                    else:
                        self.logger.debug(f"Process {pid} memory usage: {memory_usage.rss_mb:.1f}MB")
    ...
    def _terminate_process_for_memory(self, pid: int, process: psutil.Process, memory_usage: MemoryUsage):
        ...
        self.logger.warning(f"Process {pid} exceeded memory limit: "
                           f"{memory_usage.rss_mb:.1f}MB > {self.memory_limit_mb}MB, terminating")
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Process {pid} force killed after graceful termination timeout")
        ...
        if self._termination_callback:
            try:
                ...
                self.logger.error(f"Error in termination callback: {e}")
...
class PlanProcessingWatchdog:
    ...
    def __init__(self, memory_limit_mb: Optional[int] = None):
        ...
        if not PSUTIL_AVAILABLE:
            raise ImportError("psutil is required for plan processing memory monitoring")
    ...
    def _handle_worker_termination(self, event: WatchdogEvent):
        ...
        if event.termination_reason == TerminationReason.MEMORY_EXCEEDED:
            self.logger.error(f"Worker process {event.pid} terminated for exceeding memory limit "
                             f"({event.memory_usage.rss_mb:.1f}MB > {event.memory_limit_mb}MB)")
    ...
    def complete_plan_processing(self, worker_pid: int):
        ...
        self.logger.info(f"Plan processing completed successfully for worker {worker_pid}")
...
def demo_memory_watchdog():
    ...
    if not PSUTIL_AVAILABLE:
        print("psutil not available - cannot run memory watchdog demo")
        return
        
    print("Memory Watchdog Demo")
    print("=" * 40)
    ...
    print("\nMemory watchdog demo completed")
