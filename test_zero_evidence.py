...
def test_zero_evidence_support():
    ...
    print(f"   Components detected: {len(normal_result_with_evidence.components_detected)}")
    
    # Verify expectations
    assert normal_result.feasibility_score > 0.0, "Normal scoring should have positive score"
    assert normal_result.quality_tier != 'REQUIERE MAYOR EVIDENCIA', "Normal scoring should not require more evidence"
    
    assert zero_evidence_result.feasibility_score == 0.0, "Zero evidence should result in zero score"
    assert zero_evidence_result.quality_tier == 'REQUIERE MAYOR EVIDENCIA', "Zero evidence should require more evidence"
    assert len(zero_evidence_result.components_detected) == 0, "Zero evidence should detect no components"
    
    assert normal_result_with_evidence.feasibility_score > 0.0, "Non-zero evidence should score normally"
    assert normal_result_with_evidence.quality_tier != 'REQUIERE MAYOR EVIDENCIA', "Non-zero evidence should not require more evidence"
...
def test_batch_scoring_with_evidence():
    ...
    for i, (indicator, evidencia, result) in enumerate(zip(indicators, evidencia_list, results)):
    ...
    assert results[0].quality_tier == 'REQUIERE MAYOR EVIDENCIA', "First result should require more evidence"
    
    # Other results should score normally (though may still be low due to missing components)
    assert results[1].quality_tier != 'REQUIERE MAYOR EVIDENCIA', "Second result should not require more evidence"
    assert results[2].quality_tier != 'REQUIERE MAYOR EVIDENCIA', "Third result should not require more evidence"
