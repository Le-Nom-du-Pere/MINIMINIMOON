...
class TestDocumentSegmenter(unittest.TestCase):
    ...
    def test_long_text_segmentation(self):
        ...
        segments_in_char_range = sum(1 for seg in segments 
                                   if self.segmenter.target_char_min <= seg['metrics'].char_count <= self.segmenter.target_char_max)
        
        segments_with_target_sentences = sum(1 for seg in segments 
                                           if seg['metrics'].sentence_count == self.segmenter.target_sentences)
        
        segments_near_target = sum(1 for seg in segments
        ...
        self.assertGreater(max(segments_in_char_range, segments_with_target_sentences, segments_near_target), 0)
    ...
    def test_dual_criteria_logic(self):
        ...
        for i, test_text in enumerate(test_cases):
            with self.subTest(case=i):
                ...
                for segment in segments:
                    metrics = segment['metrics']
                    self.assertLessEqual(metrics.char_count, self.segmenter.max_segment_chars)
    ...
    def test_character_distribution_buckets(self):
        ...
        test_segments = [
            {'metrics': SegmentMetrics(char_count=400, sentence_count=2, word_count=50, token_count=50)},
            {'metrics': SegmentMetrics(char_count=650, sentence_count=3, word_count=80, token_count=80)},
            {'metrics': SegmentMetrics(char_count=800, sentence_count=3, word_count=100, token_count=100)},
            {'metrics': SegmentMetrics(char_count=1100, sentence_count=4, word_count=140, token_count=140)},
        ]
    ...
    def test_large_segment_splitting(self):
        ...
        very_long_text = " ".join(["This is sentence number " + str(i) + " with additional content to make it longer." for i in range(50)])
        ...
        for segment in segments:
            self.assertLessEqual(segment['metrics'].char_count, self.segmenter.max_segment_chars)
    ...
    def test_emergency_fallback(self):
        ...
        for segment in segments:
            self.assertLessEqual(segment['metrics'].char_count, 
                                (self.segmenter.target_char_min + self.segmenter.target_char_max) // 2 + 50)  # Allow some flexibility
