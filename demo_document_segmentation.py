...
def main():
    ...
    print(
        f"Configuration: {segmenter.target_char_min}-{segmenter.target_char_max} chars, {segmenter.target_sentences} sentences\n"
    )
    ...
    for doc_type, text in demo_texts.items():
        ...
        print(
            f"  Character range success: {report['summary']['char_range_success_rate']:.1f}%"
        )
        print(
            f"  Sentence target success: {report['summary']['sentence_target_success_rate']:.1f}%"
        )
        print(
            f"  Consistency score: {report['quality_indicators']['consistency_score']:.3f}"
        )
    ...
    configurations = [
        ...
        {
            "name": "Extended",
            "target_char_min": 1000,
            "target_char_max": 1300,
            "target_sentences": 4,
        },
    ]
