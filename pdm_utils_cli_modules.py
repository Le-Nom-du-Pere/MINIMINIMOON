...
class ExplanationTracer:
    ...
    def generate_explanations(
        ...
        if any([contradictions, competence_issues, agenda_gaps]):
            explanations.append(self._generate_summary(
                len(contradictions),
                len(competence_issues),
                len(agenda_gaps)
            ))
    ...
    def _explain_contradictions(self, contradictions: List[Any]) -> str:
        ...
        high_risk = [c for c in contradictions 
                     if hasattr(c, 'risk_level') and 'high' in str(c.risk_level).lower()]
        
        explanation = f"Se detectaron {len(contradictions)} contradicciones en el documento:\n"
        
        if high_risk:
            explanation += f"- {len(high_risk)} de alto riesgo que requieren atención inmediata\n"
    ...
    def _explain_competences(self, issues: List[Any]) -> str:
        ...
        overreach = [i for i in issues 
                     if (isinstance(i, dict) and 'overreach' in i.get('type', '')) or
                        (hasattr(i, 'competence_type') and 'overreach' in str(i.competence_type))]
    ...
    def _explain_agenda(self, gaps: List[Any]) -> str:
        ...
        explanation = f"Se encontraron {len(gaps)} brechas en la alineación de agenda:\n"
        ...
        if any(severities.values()):
            ...
            if severities["high"] > 0:
                explanation += f"  • Alta: {severities['high']} (requieren acción inmediata)\n"
    ...
    def _generate_summary(
        ...
        if total == 0:
            summary += "✅ No se encontraron problemas significativos en el PDM analizado."
        elif total <= 5:
...
def scan(
    ...
    competence_matrix: Optional[Path] = typer.Option(
        None,
        "--matrix", "-m",
        help="Archivo JSON con matriz de competencias"
    ),
    sectors: Optional[List[str]] = typer.Option(
        None,
        "--sector", "-s",
        help="Sectores a analizar"
    ),
    light_mode: bool = typer.Option(
        False,
        "--light", "-l",
        help="Usar modo ligero (modelos más pequeños)"
    ),
    ...
    if not sectors:
        sectors = ["salud", "educacion", "agua", "ambiente", 
                  "movilidad", "seguridad", "cultura", "vivienda"]
...
def report(
    ...
    format: str = typer.Option(
        "markdown",
        "--format", "-f",
        help="Formato del reporte (markdown/html/json)"
    ),
    output: Optional[Path] = typer.Option(
        None,
        "--output", "-o",
...
def explain(
    ...
    item_id: Optional[str] = typer.Option(
        None,
        "--id", "-i",
        help="ID específico del hallazgo a explicar"
    )
):
    """
    ...
    if item_id:
        ...
        if item:
            ...
            console.print(f"[red]No se encontró el item con ID: {item_id}[/red]")
    else:
        # Explain all high-risk items
        high_risk_items = _get_high_risk_items(results)
        if high_risk_items:
            console.print("[bold]Explicación de hallazgos de alto riesgo:[/bold]\n")
            for item in high_risk_items[:5]:  # Limit to top 5
                _explain_item(item)
                console.print("-" * 60)
        else:
            console.print("[green]No se encontraron hallazgos de alto riesgo[/green]")
...
def _display_summary(analysis, verbose: bool):
    ...
    table.add_row(
        ...
        str(analysis.total_competence_issues),
        ""
    )
    table.add_row(
        "Brechas de Agenda",
        str(analysis.total_agenda_gaps),
        ""
    )
    ...
    # Risk score
    risk_color = "red" if analysis.risk_score > 0.7 else "yellow" if analysis.risk_score > 0.4 else "green"
    ...
    if analysis.confidence_intervals:
        ...
        console.print(f"Intervalo de Confianza (90%): [{ci[0]:.2f}, {ci[1]:.2f}]")
