...
class FactibilidadScorer:
    ...
    def update_weights(
        self, w1: float = None, w2: float = None, w3: float = None
    ) -> None:
        """
    ...
    def score_text(self, text: str, similarity_score: float = 0.0) -> Dict:
        ...
        clusters = self.pattern_detector.find_pattern_clusters(
            text, self.proximity_window
        )
        ...
        return {
            ...
            "normalized_causal_connections": causal_connections
            / max(1, segment_length),
    ...
    def _calculate_refined_score(
        self,
        similarity_score: float,
        causal_connections: int,
        segment_length: int,
        informative_length_ratio: float,
    ) -> float:
        """
    ...
    def _count_causal_connections(
        self, all_matches: Dict[str, List[PatternMatch]]
    ) -> int:
        """
    ...
    def _calculate_informative_length_ratio(self, text: str) -> float:
        ...
        informative_words = [
            word for word in words if word not in self.spanish_stopwords
        ]
        ...
        for pattern_type, pattern_matches in matches.items():
            if pattern_matches:
                ...
                if len(pattern_matches) > 1:
                    type_score += self.multiple_instance_bonus * math.log(
                        len(pattern_matches)
                    )
    ...
    def _generate_analysis(
        self, matches: Dict[str, List[PatternMatch]], clusters: List[Dict]
    ) -> Dict:
        ...
        if analysis["has_complete_cluster"]:
            analysis["strengths"].append(
                "Contiene patrones completos de línea base, metas y plazos"
            )

        if len(clusters) > 1:
            analysis["strengths"].append(
                "Múltiples grupos de patrones completos identificados"
            )
        ...
        if not analysis["has_baseline"]:
            analysis["weaknesses"].append(
                "Falta indicadores de línea base o situación inicial"
            )

        if not analysis["has_target"]:
            analysis["weaknesses"].append(
                "Falta indicadores de metas u objetivos claros"
            )

        if not analysis["has_timeframe"]:
            analysis["weaknesses"].append(
                "Falta indicadores temporales o plazos específicos"
            )

        if not analysis["has_complete_cluster"]:
            analysis["weaknesses"].append(
                "Los patrones no aparecen agrupados en el texto"
            )

        return analysis

    def score_segments(
        self, text: str, segment_size: int = 1000, overlap: int = 200
    ) -> List[Dict]:
        """
