...
class PatternDetector:
    ...
    def detect_patterns(self, text: str) -> Dict[str, List[PatternMatch]]:
        ...
        """
        return {
            "baseline": PatternDetector._find_matches(
                text, self.baseline_patterns, "baseline"
            ),
            "target": PatternDetector._find_matches(
                text, self.target_patterns, "target"
            ),
            "timeframe": PatternDetector._find_matches(
                text, self.timeframe_patterns, "timeframe"
            ),
        }

    def _find_matches(
        self, text: str, patterns: List[re.Pattern], pattern_type: str
    ) -> List[PatternMatch]:
    ...
    def _within_proximity(
        self, a: PatternMatch, b: PatternMatch, proximity_window: int
    ) -> bool:
