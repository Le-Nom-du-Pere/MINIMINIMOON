...
class TestPlanProcessingWatchdog:
    ...
    def test_plan_processing_lifecycle(self, plan_watchdog):
        ...
        with mock.patch.object(plan_watchdog.watchdog, 'register_process', return_value=True):
            with mock.patch.object(plan_watchdog.watchdog, 'start_monitoring', return_value=True):
...
class TestIntegrationScenarios:
    ...
    def test_memory_exhaustion_simulation(self, memory_watchdog, mock_psutil):
        ...
        termination_events = []
        memory_watchdog.set_termination_callback(lambda event: termination_events.append(event))
        
        # Mock process with high memory usage
        mock_process = mock_psutil.Process.return_value
        mock_process.memory_info.return_value.rss = 300 * 1024 * 1024  # 300MB > 100MB limit
        ...
        assert termination_events[0].termination_reason == TerminationReason.MEMORY_EXCEEDED
        assert 1234 not in memory_watchdog._monitored_processes
